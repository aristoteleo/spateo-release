...
def glm_fit(
    ...
    **kwargs,
    ...
    Args:
        ...
        feature_x: The key in ``.uns[glm_key]['correlation'][gene]`` that corresponds to the independent variables,
            such as ``'torsion'``, etc.
        feature_y: The key in ``.uns[glm_key]['correlation'][gene]`` that corresponds to the dependent variables,
            such as ``'expression'``, etc.
        feature_fit: The key in ``.uns[glm_key]['correlation'][gene]`` that corresponds to the fitted values,
            such as ``'mu'``, etc.
        ...
        frac: Between 0 and 1. The fraction of the data used when estimating each feature_y-value. Only valid when
            ``lowess`` is True.
