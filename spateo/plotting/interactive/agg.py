...
def contours(adata: AnnData, layer: str, colors: Optional[List] = None) -> go.Figure:
    ...
    for bin in np.unique(bins):
        if bin > 0:
            ...
            mtx[mtx > 0] = 255
            contour = cv2.findContours(mtx, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0][0].squeeze()
    ...
    figure.update_layout(
        width=bins.shape[1] * 0.2,
        height=bins.shape[0] * 0.2,
