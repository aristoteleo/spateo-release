...
def extract_coords_exp(
    sdata, batch_key="batch", cols="spatial", count_layer="count", data_format="norm1e4", ifcombat=False, if_inte=False
):
    coords_raw = {}
    exps = {}
    samples = np.unique(sdata.obs[batch_key])
    ...
    if if_inte:
        ...
        for sample_t in samples:
            idx_t = sdata.obs[batch_key] == sample_t
            coords_raw[sample_t] = (
                sdata.obsm["spatial"][idx_t] if type(cols) is not list else np.array(sdata.obs[cols][idx_t])
            )
...
def sub_data_extract(sample_list, coords_raw, exps, nodenum_t=20000, if_non_zero_center_scale=True):
    ...
    for sample_t in sample_list:
        exp_t, sub_node_idxs[sample_t] = sub_node_sum(
            coords_raw[sample_t], exps[sample_t], nodenum=nodenum_t, vis=False
        )
