...
def digitize(
    ...
    digi_layer_key: str = "digital_layer",
    digi_column_key: str = "digital_column",
    lp: float = 1,
    hp: float = 100,
) -> None:
    ...
    Args:
        ...
        digi_layer_key (str, optional): The key name to store layer digi-weights. Defaults to "digital_layer".
        digi_column_key (str, optional): The key name to store column digi-weights.
        lp (float, optional): lowest digi-weight. Defaults to 1.
        hp (float, optional): highest digi-weight. Defaults to 100.
    """
    ...
    adata.obs[digi_layer_key] = 0
    for i in range(len(adata)):
        adata.obs[digi_layer_key][i] = of_layer[int(adata.obsm[spatial_key][i, 0]), int(adata.obsm[spatial_key][i, 1])]

    of_column = calc_op_field(
        empty_field, min_line_c, max_line_c, min_line_l, max_line_l, field_border, field_mask, lp=lp, hp=hp
    )
    adata.obs[digi_column_key] = 0
    for i in range(len(adata)):
        adata.obs[digi_column_key][i] = of_column[
...
def soft_gridding(
    ...
    """Segment the area of interest into given number of layers/columns, according to
    ...
    Args:
        ...
        digi_layer_key (str, optional): The key name of layer digitization weights. Defaults to "digital_layer".
        digi_column_key (str, optional): The key name of column digitization weights. Defaults to "digital_column".
    ...
    adata.obs[grid_label_key][adata.obs[digi_layer_key] != 0] = "Grid Area"
    adata.obs[grid_label_key][adata.obs[digi_column_key] != 0] = "Grid Area"
    ...
    for i in range(len(value_list) - 1):
        adata.obs[layer_label_key] = np.where(
            (adata.obs[digi_layer_key] > value_list[i]) & (adata.obs[digi_layer_key] <= value_list[i + 1]),
            i + 1,
            adata.obs[layer_label_key],
        )
        adata.obs[grid_label_key] = np.where(
            (adata.obs[digi_layer_key] > (value_list[i] - layer_border_width / 2))
            & (adata.obs[digi_layer_key] <= (value_list[i] + layer_border_width / 2)),
            region_mask,
    ...
    for i in range(len(value_list) - 1):
        adata.obs[column_label_key] = np.where(
            (adata.obs[digi_column_key] > value_list[i]) & (adata.obs[digi_column_key] <= value_list[i + 1]),
            i + 1,
            adata.obs[column_label_key],
        )
        adata.obs[grid_label_key] = np.where(
            (adata.obs[digi_column_key] > (value_list[i] - column_border_width / 2))
            & (adata.obs[digi_column_key] <= (value_list[i] + column_border_width / 2)),
            region_mask,
            adata.obs[grid_label_key],
        )
