...
def digitize(
    ...
    """Calculate the "heat" for a closed area of interests by solving a PDE, partial differential equation, the heat
        equation. Boundary conditions are defined upon four user provided coordinates that define the direction of heat
    ...
    Args:
        ...
        pnt_xy: Corner point to define an area of interest.
            pnt_xy correspond to the minimal value in both columns and layers.
        pnt_Xy: Corner point correspond to the maximal value in columns and minimal in layers.
        pnt_xY: Corner point correspond to the minimal value in columns and maximal in layers.
        pnt_XY: Corner point correspond to the maximal value
            in both columns and layers.
        spatial_key: The key name in `adata.obsm` of the spatial coordinates. Default to "spatial".
        dgl_layer_key: The key name in `adata.obs` to store layer digital-heat (temperature). Default to "digital_layer".
        dgl_column_key: The key name to store column digital-heat (temperature).
        lp: lowest digital-heat (temperature). Defaults to 1.
        hp: highest digital-heat (temperature). Defaults to 100.
    """
...
def gridit(
    ...
    """Segment the area of interests into specific layer/column number, according to pre-calculated digitization heat
    value.

    Args:
        adata: The adata object to do grid segmentation.
        layer_num: Number of layers to segment.
        column_num: Number of columns to segment.
        lp: lowest digital-weight. Default to 1.
        hp: highest digi-weight. Default to 100.
        ...
        dgl_layer_key: The key name of layer digitization heat. Default to "digital_layer".
        dgl_column_key: The key name of column digitization heat. Default to "digital_column".
        layer_label_key: The key name to store layer labels. Default to "layer_label".
        column_label_key: The key name to store column labels. Default to "column_label".
        grid_label_key: The key name to store grid labels. Default to "grid_label".
    ...
    Returns:
        Nothing but update the adata object with the following keys in `.obs`:
         ...
         4. dgl_layer_key: this key points to layer digitization heat values.
         5. dgl_column_key: this key points to column digitization heat values.
    """
