...
def bin_adata(adata, bin_size=1):
    ...
    df = pd.DataFrame(adata.X.A, columns=adata.var_names) if issparse(adata.X) else pd.DataFrame(adata.X, columns=adata.var_names)
...
def _cal_geodesic_distance(adata, n_neighbors=30, min_dis_cutoff=2.0, max_dis_cutoff=4.0):
    ...
    b = adata[np.min(adata.obsp['spatial_distances'].A, axis=1, initial=1e10, where=np.array(adata.obsp['spatial_distances'].A>0))<=min_dis_cutoff]
...
def cal_wass_dis_bs(
    ...
    mean_std_df = pd.DataFrame({'mean': w_df.groupby('gene_id')['Wasserstein_distance'].mean().to_list(), 
