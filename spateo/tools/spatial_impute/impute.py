...
from spateo.preprocessing.find_neighbors import construct_pairwise, normalize_adj
...
def get_aug_feature(adata: anndata.AnnData,
...
class STGNN():
    ...
    def train_STGNN(self):
        if self.add_regularization:
            ...
            self.adata_output.obsm['ReX_reg'] = emb_regularization

        else:
            model = Trainer(self.adata.copy(), device=self.device)
            emb = model.train()
            self.adata_output.obsm['ReX'] = emb
