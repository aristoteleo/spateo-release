...
def niches(
    ...
    if len(expressed_ligand) == 0:
        raise ValueError(f"No intersected ligand between your adata object"
                         f" and lr_network dataset.")
    ...
    if len(expressed_receptor) == 0:
        raise ValueError(f"No intersected receptor between your adata object"
                         f" and lr_network dataset.")
    ...
    if spatial_neighbors not in adata.uns.keys():
        raise ValueError(f"No spatial_key {spatial_neighbors} exists in adata,"
                         f"using 'dyn.tl.neighbors' to calulate the spatial neighbors first.")
    if spatial_distances not in adata.obsp.keys():
        raise ValueError(f"No spatial_key {spatial_distances} exists in adata,"
...
def predict_ligand_activities(
    ...
    expressed_genes_sender = np.array(adata.var_names)[np.count_nonzero(adata[sender_cells, :].X.A, axis=0)/len(sender_cells)>0.01].tolist()
    expressed_genes_receiver = np.array(adata.var_names)[np.count_nonzero(adata[receiver_cells, :].X.A,axis=0)/len(receiver_cells)>0.01].tolist()
    ...
    lr_network_expressed = lr_network[lr_network['from'].isin(expressed_ligand) & lr_network['to'].isin(expressed_receptor)]
