...
class Lasso():
    ...
    def vi_plot(
        ...
        if group and group_color:
            ...
            f = go.FigureWidget([go.Scatter(x= df['spatial_0'], y = df['spatial_1'], mode = 'markers',marker_color=df["color"])])
            scatter = f.data[0]
            ...
            t = go.FigureWidget([go.Table(
                header=dict(values=['group_ID','labels','spatial_0','spatial_1'],
                            fill = dict(color='#C2D4FF'),
                            align = ['left'] * 5),
                cells=dict(values=[df[col] for col in ['group_ID','labels','spatial_0','spatial_1']],
                        fill = dict(color='#F5F8FF'),
                        align = ['left'] * 5))])

            def selection_fn(trace,points,selector):

                t.data[0].cells.values = [df.loc[points.point_inds][col] for col in ['group_ID','labels','spatial_0','spatial_1']]
