...
_split = lambda nx, x, chunk_size, dim: (
    torch.split(x, chunk_size, dim) if nx_torch(nx) else np.array_split(x, chunk_size, axis=dim)
)
...
_repeat_interleave = lambda nx, x, repeats, axis: (
    torch.repeat_interleave(x, repeats, dim=axis) if nx_torch(nx) else np.repeat(x, repeats, axis)
)
