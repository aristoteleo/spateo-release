...
def get_P(
    ...
    GeneDistMat: Union[np.ndarray, torch.Tensor, SparseTensor],
    SpatialDistMat: Union[np.ndarray, torch.Tensor, SparseTensor],
    ...
    if outlier_variance is None:
        exp_SpatialMat = _kernel_exp(SpatialDistMat, sigma2)
    else:
        exp_SpatialMat = _kernel_exp(SpatialDistMat, sigma2 / outlier_variance)
    # calculate spatial_P
    spatial_term1 = exp_SpatialMat * (_mul(nx)(alpha, nx.exp(-Sigma / sigma2))).unsqueeze(-1)
    spatial_term1 = spatial_term1 * label_mask if label_mask is not None else spatial_term1
    ...
    K_NB_spatial = spatial_P.sum(0)
    Sp_spatial = spatial_P.sum()
    del spatial_P
    ...
    # calculate sigma2_P
    term1 = exp_SpatialMat * (_mul(nx)(alpha, nx.exp(-Sigma / sigma2))).unsqueeze(-1)
    term1 = term1 * label_mask if label_mask is not None else term1
    sigma2_P = term1 / (_unsqueeze(nx)(term1.sum(0), 0) + 1e-8)
    sigma2_P = sigma2_P * spatial_inlier.unsqueeze(0)
    K_NA_sigma2 = sigma2_P.sum(1)
    K_NB_sigma2 = sigma2_P.sum(0)
    Sp_sigma2 = sigma2_P.sum()
    sigma2_temp = _sparse_mul_same(sigma2_P, SpatialDistMat).sum() / (D * Sp_sigma2)
    # sigma2_temp = (sigma2_P * SpatialDistMat).sum() / (D * Sp_sigma2)
    del sigma2_P
    
    # empty_cache(device=device)
    # calculate P
    exp_GeneMat = _kernel_exp(GeneDistMat, beta2)
    term1 = term1 * exp_GeneMat
    ...
    assignment_results = {
        ...
        'K_NB_spatial': K_NB_spatial,
        'K_NA_sigma2': K_NA_sigma2,
        'K_NB_sigma2': K_NB_sigma2,
        'Sp': Sp,
...
def BA_align_sparse(
    ...
    if SVI_mode:
        ...
        # sub_label_mask = label_mask[:, randIdx] if label_mask is not None else None  # NA x batch_size
    ...
    for iter in iteration:
        ...
        if SVI_mode:
            ...
            P, assignment_results = get_P_sparse(
                ...
                label_transfer_prior=label_transfer_prior,
                # label_mask=sub_label_mask,
            )
        else:
            P, assignment_results  = get_P(
                XnAHat=XAHat,
                XnB=coordsB,
                labelA=labelA,
                labelB=labelB,
        ...
        if (sigma2 < 0.015) or (iter > 80):
            if SVI_mode:
                ...
                PXB_term = (
                    step_size * (spmm(torch.vstack((P.storage._row, P.storage._col)),P.storage._value,P.storage._sparse_sizes[0],P.storage._sparse_sizes[1],randcoordsB) - nx.einsum("ij,i->ij", RnA, K_NA))
                ...
                SigmaDiag = sigma2 * nx.einsum("ij->i", nx.einsum("ij,ji->ij", U, term1))
                Coff = _dot(nx)(term1, (spmm(torch.vstack((P.storage._row, P.storage._col)),P.storage._value, P.storage._sparse_sizes[0],P.storage._sparse_sizes[1],coordsB) - nx.einsum("ij,i->ij", RnA, K_NA)))
                VnA = _dot(nx)(
                    U,
                    Coff,
                )
        ...
        if SVI_mode:
            A = -(
                _dot(nx)(PXA.T, t)
                + _dot(nx)(coordsA.T, nx.einsum("ij,i->ij", VnA, K_NA) - spmm(torch.vstack((P.storage._row, P.storage._col)),P.storage._value,P.storage._sparse_sizes[0],P.storage._sparse_sizes[1],randcoordsB))
                + 2
                * lambdaReg
                * sigma2
            ...
            A = -(
                _dot(nx)(PXA.T, t)
                + _dot(nx)(coordsA.T, nx.einsum("ij,i->ij", VnA, K_NA) - spmm(torch.vstack((P.storage._row, P.storage._col)),P.storage._value,P.storage._sparse_sizes[0],P.storage._sparse_sizes[1],coordsB))
                + 2
                * lambdaReg
                * sigma2
                * (_dot(nx)(PCXC.T, t) - _dot(nx)(nx.einsum("ij,i->ij", inlier_A, inlier_P[:, 0]).T, inlier_B))
        ...
        sigma2 = nx.maximum(
            (
                assignment_results["sigma2_temp"]
