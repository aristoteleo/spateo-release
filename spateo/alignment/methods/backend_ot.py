...
class TorchBackend(Backend):
    ...
    def median(self, a, axis=None):
        ...
        # Else, use numpy
        warnings.warn(
            "The median is being computed using numpy and the array has been detached " "in the Pytorch backend."
        )
        a_ = self.to_numpy(a)
...
class CupyBackend(Backend):  # pragma: no cover
    ...
    def __init__(self):
        ...
        self.__type_list__ = [cp.array(1, dtype=cp.float32), cp.array(1, dtype=cp.float64)]
    ...
    def coo_matrix(self, data, rows, cols, shape=None, type_as=None):
        ...
        if type_as is None:
            ...
            with cp.cuda.Device(type_as.device):
                return cupyx.scipy.sparse.coo_matrix((data, (rows, cols)), shape=shape, dtype=type_as.dtype)
...
class TensorflowBackend(Backend):
    ...
    def __init__(self):
        ...
        self.__type_list__ = [tf.convert_to_tensor([1], dtype=tf.float32), tf.convert_to_tensor([1], dtype=tf.float64)]
    ...
    def median(self, a, axis=None):
        warnings.warn(
            "The median is being computed using numpy and the array has been detached " "in the Tensorflow backend."
        )
        a_ = self.to_numpy(a)
        a_median = np.median(a_, axis=axis)
        return self.from_numpy(a_median, type_as=a)
    ...
    def coo_matrix(self, data, rows, cols, shape=None, type_as=None):
        if shape is None:
            shape = (self._convert_to_index_for_coo(rows), self._convert_to_index_for_coo(cols))
        if type_as is not None:
            data = self.from_numpy(data, type_as=type_as)

        sparse_tensor = tf.sparse.SparseTensor(indices=tnp.stack([rows, cols]).T, values=data, dense_shape=shape)
        # if type_as is not None:
        ...
        for device_context in device_contexts:
            with device_context:
                for type_as in self.__type_list__:
                    ...
                    _ = res.numpy()
                    t1 = time.perf_counter()
                    key = ("Tensorflow", self.device_type(inputs[0]), self.bitsize(type_as))
                    results[key] = (t1 - t0) / n_runs
